...
public class Autocomplete {
   ...
   public static class TrieAutocomplete implements Autocompletor {
      ...
      public TrieAutocomplete(String[] terms, double[] weights) {
         ...
         /*
         topMatches("ca", 2);
         String answer = topMatch("ca");
         System.out.println("the top match for \"ca\" is" + answer);
         */
      }
      ...
      private void add(String word, double weight) {
         ...
         if (curr.isWord)  {
            ...
            while(curr.parent != null){
               if(curr.parent.mySubtreeMaxWeight <= curr.mySubtreeMaxWeight){
                  curr.mySubtreeMaxWeight = weight;
               }
      ...
      public Iterable<String> topMatches(String prefix, int k) {
         ...
         Node curr = myRoot;
         PriorityQueue<Node> queue = new PriorityQueue<Node>(new Node.ReverseSubtreeMaxWeightComparator()); //middle ground queue
         ArrayList<Node> topMatches = new ArrayList<Node>(); 
         ArrayList<String> result = new ArrayList<String>();
         //for loop iterates to end of prefix
         for (int j = 0; j < prefix.length(); j++)   {
            
            //while (curr.children.containsKey(prefix.charAt(j)))  {
            curr = curr.children.get(prefix.charAt(j));
            if (curr == null) {
               return result; //empty iterable
            }
                  //curr points to node containing last letter of input prefix
               //System.out.println("curr is " + curr.myInfo);
            //}
                             
         }
         queue.add(curr); 
         //adds all options to qu
         double queueMax = queue.peek().mySubtreeMaxWeight;
         double listMax = 0;
         for (int i = 0; i < queue.size(); i++)   {
            if (topMatches.size() >= k && queueMax < listMax) {
            ...
            for(Node children : curr.children.values()){
               queue.add(children);
            }
            ...
            if (topMatches.size() > 0) {     
               listMax = topMatches.get(0).getWeight();
            }
         }
         ...
         for (int i = 0; i < size; i++)  {
            result.add(topMatches.get(i).getWord());
         }
         
         
         //System.out.println("curr's weight is " + curr.getWeight());
      ...
      public String topMatch(String prefix) {
         ...
         for (int i = 0; i < prefix.length(); i++)   {
            while (curr.children.containsKey(prefix.charAt(i)))  {
         ...
         //similar to topmatches but only one item
         while (curr.mySubtreeMaxWeight != curr.getWeight())   {
            //adds all children in order  of priority
            queue.addAll(curr.children.values());
            //removes head - one with largest weight
            curr = queue.remove();
         }
         //System.out.println("curr's weight is " + curr.getWeight());
